...
class From1svgTo1svgFlatten(FrozenClass):
    ...
    def getAllPathsDico(self, doc):
        ...
        for p in doc.getElementsByTagName('path'): 
            if p.parentNode.tagName != "marker":
    ...
    def run(self):
        ...
        for g in allGroupsDico:
            if 'display:none' in allGroupsDico[g]['obj'].getAttribute('style'):
                allGroupsDico[g]['obj'].setAttribute('style', 'display:none')
            else:                
        ...
        with open(self.svg_flatten_res_filename, 'w', encoding='utf-8') as outfile: 
            svgInDoc.writexml(outfile, indent='', addindent='', newl='', encoding='UTF-8')
        crlf = b'\r\n'
        cr = b'\n'
